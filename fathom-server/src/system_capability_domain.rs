...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct DescribeCapabilityDomainArgs {
...
pub(crate) async fn execute_action(
    ...
    match action_name {
        ...
        "describe_capability_domain" => {
            Some(execute_describe_capability_domain(context, args_json))
        }
...
fn execute_describe_capability_domain(
    context: &ExecutionContext,
    args_json: &str,
) -> ActionOutcome {
    let args = match parse_args::<DescribeCapabilityDomainArgs>(
        args_json,
        "system__describe_capability_domain",
    ) {
        Ok(args) => args,
        Err(error) => return failure("system__describe_capability_domain", error),
    };
    let capability_domain_id = args.capability_domain_id.trim();
    if capability_domain_id.is_empty() {
        return failure(
            "system__describe_capability_domain",
            "capability_domain_id must be non-empty".to_string(),
        );
    }
    if !context
        .engaged_capability_domain_ids
        .iter()
        .any(|id| id == capability_domain_id)
    {
        return failure(
            "system__describe_capability_domain",
            format!("capability domain `{capability_domain_id}` is not activated in this session"),
        );
    }

    match capability_domains::describe_capability_domain(capability_domain_id) {
        Some(payload) => success("system__describe_capability_domain", payload),
        None => failure(
            "system__describe_capability_domain",
            format!("unknown capability domain `{capability_domain_id}`"),
        ),
    }
}
...
mod tests {
    ...
    async fn system_get_context_returns_payload() {
        ...
        let context = ExecutionContext {
            ...
            engaged_capability_domain_ids: vec!["filesystem".to_string(), "system".to_string()],
        };
    ...
    #[tokio::test]
    async fn system_get_context_includes_capability_domain_summaries() {
        let runtime = Runtime::new(2, 10);
        let context = ExecutionContext {
            session_id: "session-1".to_string(),
            active_agent_id: "agent-1".to_string(),
        ...
        let payload: Value = serde_json::from_str(&outcome.message).expect("valid json payload");
        let capability_domains = payload["data"]["activated_capability_domains"]
            .as_array()
            .expect("activated_capability_domains must be an array");

        assert!(!capability_domains.is_empty());
    }
    ...
    async fn system_get_context_excludes_policy_fields() {
        ...
        assert!(data["activated_capability_domains"].is_array());
    }

    #[tokio::test]
    async fn system_describe_capability_domain_requires_activation() {
        let runtime = Runtime::new(2, 10);
        let context = ExecutionContext {
            session_id: "session-1".to_string(),
            active_agent_id: "agent-1".to_string(),
            ...
            active_agent_spec_version: 1,
            participant_user_updated_at: [("user-1".to_string(), 123)].into_iter().collect(),
            engaged_capability_domain_ids: vec!["system".to_string()],
        };
        ...
        let outcome = execute_action(
            ...
            &context,
            "describe_capability_domain",
            r#"{"capability_domain_id":"filesystem"}"#,
        )
        .await
        .expect("should dispatch describe_capability_domain");
        assert!(!outcome.succeeded);
    }

    #[tokio::test]
    async fn system_describe_capability_domain_returns_action_inventory() {
        let runtime = Runtime::new(2, 10);
        let context = ExecutionContext {
            session_id: "session-1".to_string(),
            active_agent_id: "agent-1".to_string(),
    ...
    #[tokio::test]
    async fn system_describe_capability_domain_includes_filesystem_path_rules() {
        let runtime = Runtime::new(2, 10);
        let context = ExecutionContext {
            session_id: "session-1".to_string(),
            active_agent_id: "agent-1".to_string(),
