# fathom-capability-domain-shell

Shell capability-domain implementation for Fathom's `env + action` runtime model.

This crate provides the canonical `shell` capability domain and executes:
...
- Commands run under a base path (`base_path`) defined by capability-domain state.
...
Use `.` for the capability-domain root directory.
