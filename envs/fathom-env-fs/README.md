...
Filesystem environment implementation for Fathom's `env + action` runtime model.

This crate provides the canonical `filesystem` environment and executes these actions:
...
`fathom-env-fs` gives agents safe, non-destructive file operations rooted at a configured base path.
...
Use `.` to reference the environment root.
